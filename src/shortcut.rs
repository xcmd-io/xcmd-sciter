use com::{self, CLSID_ShellLink, ComLibrary, ComPtr, IPersistFile, IShellLinkW, SLGP_SHORTPATH};
...
pub fn read_link(link: &Path, hwnd: *const _HWINDOW) -> PathBuf {
	...
	let shell_link: ComPtr<IShellLinkW> =
		ComPtr::create_instance(&CLSID_ShellLink).expect("IShellLinkW");
...
pub fn update_link(link: &Path, target: &Path) {
	...
	invoke!(shell_link, SetPath(com::to_wchar(target.as_ref()))).expect("set path");
